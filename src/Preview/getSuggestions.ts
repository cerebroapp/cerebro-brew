...
const getBrewPackagesFzf = async () => {
  ...
  const casksRequest = await fetch(
    "https://formulae.brew.sh/api/versions-casks.json"
  );
  const caskPackages = casksRequest.ok
    ? Object.keys(await casksRequest.json())
    : [];

  const formulaeRequest = await fetch(
    "https://formulae.brew.sh/api/versions-formulae.json"
  );
  const formulaePackages = formulaeRequest.ok
    ? Object.keys(await formulaeRequest.json())
    : [];
